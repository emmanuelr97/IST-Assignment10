...
class DNAC_Manager:
    def __init__(self):
        """Initialize DNAC manager with empty token"""
        self.token = None
    
    def get_auth_token(self, display_token=False):
        """Authenticate to DNA Center and store access token"""
        try:
    ...
    def get_network_devices(self):
        """Retrieve list of all network devices"""
        if not self.token:
    ...
    def display_devices(self, devices):
        """Display network devices in table format"""
        if not devices:
    ...
    def get_device_interfaces(self, device_ip):
        """Retrieve interfaces for specific device"""
        if not self.token:
            print("⚠️ Please authenticate first!")
            return None
            
        try:
            # Locate device by management IP
            ...
            # Retrieve interfaces using device ID
    ...
    def display_interfaces(self, interfaces):
        """Display interface information in table format"""
        if not interfaces:
...
def main():
    """Main program execution with interactive menu"""
    print("\n" + "="*50)
